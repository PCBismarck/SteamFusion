...
public static class Ipc
{
    ...
    public static async Task ServeAsync(Func<Request, Task<Reply>> handler, CancellationToken ct)
    {
        ...
        {
            var pipe = new NamedPipeServerStream(PipeName, PipeDirection.InOut, 8, PipeTransmissionMode.Byte,
    ...
    private static async Task HandleAsync(NamedPipeServerStream pipe, Func<Request, Task<Reply>> handler, CancellationToken ct)
    {
        ...
        using (var timeout = CancellationTokenSource.CreateLinkedTokenSource(ct))
        {
            ...
            catch (Exception ex) when (ex is IOException or OperationCanceledException or System.Text.Json.JsonException or InvalidDataException) { }
        }
    }
