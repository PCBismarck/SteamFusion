...
export function installVirtualLibrary(options: {
    ...
    let status: HTMLElement | undefined, search: HTMLInputElement | undefined, more: HTMLButtonElement | undefined;
    ...
    function close() { shown = false; if (panel) panel.hidden = true; button?.setAttribute('aria-expanded', 'false'); drawer?.remove(); drawer = undefined; }
    function position() {
    ...
    function render() {
        ...
        grid.replaceChildren(); drawer?.remove(); drawer = undefined;
        ...
        for (const game of matches.slice(0, limit)) {
        ...
        more!.hidden = matches.length <= limit; more!.textContent = `显示更多（${Math.min(limit, matches.length)} / ${matches.length}）`;
    }
    function open() {
    ...
    function mount(navigation: HTMLElement) {
        ...
        search.value = query; search.addEventListener('input', () => { query = search!.value; limit = 60; render(); });
        ...
        const scroll = el('div', 'sfvl-scroll'); grid = el('div', 'sfvl-grid'); more = action('显示更多', () => { limit += 60; render(); }, 'sfvl-more'); scroll.append(grid, more);
