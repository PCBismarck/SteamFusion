...
try {
    ...
    foreach ($sfProject in @('SteamFusion.App', 'SteamFusion.Cli')) {
        & dotnet publish ('src/' + $sfProject) -c Release -r win-x64 --self-contained true -o $sfOutput --nologo
    ...
    New-Item -ItemType Directory -Path (Join-Path $sfOutput 'Millennium') -Force | Out-Null
    Copy-Item integrations/millennium/SteamFusion.star (Join-Path $sfOutput 'Millennium') -Force
